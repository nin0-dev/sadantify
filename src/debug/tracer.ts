...
export const beginTrace = !(IS_DEV || IS_REPORTER) ? noop : (name: string, ...args: any[]) => {
    if (name in traces) {
...
export const finishTrace = !(IS_DEV || IS_REPORTER) ? noop : (name: string) => {
...
export const traceFunction = !(IS_DEV || IS_REPORTER) ? noopTracer : <F extends Func>(name: string, f: Func, mapper?: TraceNameMapper<F>): F => {
